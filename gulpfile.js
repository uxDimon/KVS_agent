...
//fonts
function fonts() {
	return gulp.src(paths.m18images.src).pipe(gulp.dest(paths.m18images.dest));
}
...
// просто работаем
gulp.task("default", gulp.series(gulp.parallel(styles, templates, images, scripts, fonts, m18images), gulp.parallel(watch, server)));

// контрольная сборка на продакшен
gulp.task("build", gulp.parallel(clean, gulp.parallel(styles, templates, images, scripts, fonts, m18images)));
